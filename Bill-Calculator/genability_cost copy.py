...
import resstock.GenabilityHack as GenabilityHack
importlib.reload(GenabilityHack)
from resstock.GenabilityHack import get_tariff_gen, calculate_bill_electric
...
def electric_bill(elecTariff, state, zip_codes, buildings, weights, upgrade=0):
    ...
    annual_bill = 0
    # Get tariff once
    tariff, name = get_tariff_gen(elecTariff, zip_codes[0], buildings[0])
    for zip_code, building in zip(zip_codes, buildings):
        ...
        costs_breakdown.write_csv(f"Cost_Detail/{state}/{building['bldg_id'][0]}/gas-{upgrade}-{elecTariff}.csv")
...
def genability_costs(elecTariffs, gasTariff, state, utility, gas_utility="",kwargs={}, upgrades=0):
    ...
    if not gas_utility:
        ...
        building_ids, zip_codes, elec_weights, gas_weights, gas_utilities = map(list, zip(*chosen_utility_group))
    ...
    annual_gas_bill.update([gas_bill(gasTariff, state, gas_utility, buildings, gas_weights)])
    if upgrades:
        for upgrade,buildings_upgrade in buildings_upgrades:
            annual_gas_bill.update([gas_bill(gasTariff, state, gas_utility, buildings_upgrade, gas_weights, upgrade)])

    for elecTariff in elecTariffs:
        annual_electric_bill.update([electric_bill(elecTariff, state, zip_codes, buildings, elec_weights)])
        if upgrades:
            for upgrade,buildings_upgrade in buildings_upgrades:
                annual_electric_bill.update([electric_bill(elecTariff, state, zip_codes, buildings_upgrade, elec_weights, upgrade)])
